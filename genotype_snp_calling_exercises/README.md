...
# set environment variables #for HC oysters data
...
#CICHREF=/home/projects/dp_00007/people/hmon/AngsdPopStruct/01_infofiles/fileOegenome10scaffoldC3G.fasta
...
which I've provided a copy of (skip the following step if you completed the bcftools filtering):
		...
		The VCF is in compressed binary fomat to save space so we'll have to use bcftools to view it. Let's have a look at the annotations that we can use to filter on. The various VCF statistics are defined in the VCF header. You can print just the head with bcftools view -h $DIR/output/calmas_allsites.bcf.gz.
		...
		Click here for the plotStatDist.R code
